...
- Customer `/request-wash` includes Shipbubble as the default preferred courier provider and sends `preferredProvider` to the backend. Selectable courier providers are `SHIPBUBBLE`, `RELAY`, and `BOLT`. Relay and Bolt requests require tapping `Use my location` so exact pickup coordinates are sent with the order. For home-delivery requests, the backend immediately attempts pickup dispatch and returns the order with any courier tracking details. Customer `/orders` shows courier delivery jobs with provider, leg, status, external reference, and tracking links returned by Shipbubble, Relay, or Bolt.
