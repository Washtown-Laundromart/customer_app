...
- Paystack callbacks land on `/orders`. If the customer is not currently authenticated, `/orders` redirects to `/auth?returnTo=/orders?...`; after login/register, `/auth` returns to that preserved URL so payment verification can complete instead of dropping the customer on the auth screen.
- `/orders` polls live orders every 15 seconds so Paystack verification/status changes appear without manual refresh. It keeps the selected/current wash order detail UI at the top and paginates the customer order list at 6 orders per page with date/time shown on each order. Clicking an order changes only the detail panel; the list remains sorted newest to oldest.
...
- Return delivery payment is separate from dispatch. When branch/admin requests return delivery for a ready order, the customer receives an in-app/email Paystack link for the quoted delivery fee. After payment verification, the backend automatically creates the courier delivery and the customer orders page shows the tracking link.
- Customer `/request-wash` includes Shipbubble as the default preferred courier provider and sends `preferredProvider` to the backend. The only selectable courier providers are `SHIPBUBBLE` and `RELAY`; unsupported provider integrations were removed while API access is uncertain. Relay requests require tapping `Use my location` so exact pickup coordinates are sent with the order. For home-delivery requests, the backend immediately attempts pickup dispatch and returns the order with any courier tracking details. Customer `/orders` shows courier delivery jobs with provider, leg, status, external reference, and tracking links returned by Shipbubble or Relay.
- Customer `/orders` shows a retry pickup dispatch button for pickup delivery jobs with `dispatch_failed` or Relay `duplicate_active_pickup`, calling `POST /api/orders/:id/pickup/retry`.
- Customer `/orders` shows courier rider name/phone when Relay or Shipbubble supplies it through delivery webhooks; until then it shows `Rider details pending`.
- After requesting a wash, the success toast includes the pickup delivery fee when the courier dispatch succeeds, and `/orders` shows pickup/return delivery fees on each courier tracking card.
